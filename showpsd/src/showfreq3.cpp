...
int   write_sig_file(const char* filename, int valid[NUM_FRAMES], int max_indices[NUM_FRAMES], float breath_signal[NUM_FRAMES]);
...
int main(int argc, char* argv[])
{
    ...
    extract_breath_signal(filtered_frames, sensor.get_num_samples(), max_indices, breath_signal, valid);
    ...
    write_sig_file("BREATH_SIGNAL", valid, max_indices, breath_signal);
...
void extract_breath_signal(float filtered_frames[NUM_FRAMES][MAX_FRAME_SIZE], int num_samples, int max_indices[NUM_FRAMES], float breath_signal[NUM_FRAMES], int valid[NUM_FRAMES])
{
	...
	for (int i = 0; i < NUM_FRAMES; i++) {
        ...
        max_indices[i] = 0;
        for (int j = 1; j < num_samples; j++) {
            int x = fabsf(filtered_frames[i][j]);
        ...
        // A max above MAX_THRESHOLD was not found in this frame
            ...
            // A position was note found. Prepare to go in LOST mode. 
            lost_counter++;
